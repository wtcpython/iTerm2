...
class AddClippingPanel: NSObject {
    ...
    func present(over parentWindow: NSWindow,
        ...
        let panel = NSPanel(contentRect: NSRect(x: 0, y: 0, width: 420, height: 290),
                            ...
                            backing: .buffered,
                            defer: true)
        panel.title = NSLocalizedString("New Clipping", comment: "UI")
    ...
    private func buildContent(in container: NSView) {
        ...
        let cancel = NSButton(title: NSLocalizedString("Cancel", comment: "UI"), target: self, action: #selector(cancelClicked))
        ...
        let add = NSButton(title: NSLocalizedString("Add", comment: "UI"), target: self, action: #selector(addClicked))
