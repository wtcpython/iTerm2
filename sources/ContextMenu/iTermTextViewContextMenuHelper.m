...
- (NSMenu *)timestampContextMenuWithEvent:(NSEvent *)event
    ...
    if (baseline != clickedTime) {
        NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Set Baseline for Relative Timestamps", @"Context menu")
    ...
    if (baseline != 0) {
        NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Disable Relative Timestamps", @"Context menu")
                                                      action:@selector(setTimestampBaseline:)
                                               keyEquivalent:@""];
        item.target = self;
...
- (NSMenu *)contextMenuWithEvent:(NSEvent *)event {
    ...
    if (mark.name) {
        ...
        NSMenuItem *removeItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Remove Named Mark", @"Context menu") action:@selector(removeNamedMark:) keyEquivalent:@""];
    ...
    if (mark && mark.hasNonEmptyCommand) {
        NSMenuItem *markItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Command Info", @"Context menu")
...
- (void)addFoldUnfoldMenuItemForLine:(int)y contextMenu:(NSMenu *)contextMenu {
    ...
    if (foldMark) {
        NSMenuItem *markItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Unfold", @"Context menu")
        ...
        NSMenuItem *markItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Fold", @"Context menu")
...
- (NSMenu *)menuAtCoord:(VT100GridCoord)coord {
    ...
    if (imageInfo) {
        ...
        if (imageInfo.broken) {
            entryDicts =
                @[ @{ @"title": NSLocalizedString(@"Save File As…", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(saveImageAs:)) },
                   @{ @"title": NSLocalizedString(@"Copy File", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(copyImage:)) },
                   @{ @"title": NSLocalizedString(@"Open File", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(openImage:)) },
                   @{ @"title": NSLocalizedString(@"Inspect", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(inspectImage:)) } ];
        } else {
            entryDicts =
                @[ @{ @"title": NSLocalizedString(@"Save Image As…", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(saveImageAs:)) },
                   @{ @"title": NSLocalizedString(@"Copy Image", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(copyImage:)) },
                   @{ @"title": NSLocalizedString(@"Open Image", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(openImage:)) },
                   @{ @"title": NSLocalizedString(@"Inspect", @"Context menu"),
                      @"selector": NSStringFromSelector(@selector(inspectImage:)) } ];
        }
        if (imageInfo.animated || imageInfo.paused) {
            ...
            if (imageInfo.paused) {
                entryDicts = [entryDicts arrayByAddingObject:@{ @"title": NSLocalizedString(@"Resume Animating", @"Context menu"),
                                                                @"selector": selector }];
            } else {
                entryDicts = [entryDicts arrayByAddingObject:@{ @"title": NSLocalizedString(@"Stop Animating", @"Context menu"),
                                                                @"selector": selector }];
            }
        }
            ...
            for (iTermSelectionReplacement *replacement in replacements) {
                ...
                switch (replacement.kind) {
                    case iTermSelectionReplacementKindJson:
                        item.title = NSLocalizedString(@"Replace with Pretty-Printed JSON", @"Context menu");
                        item.action = @selector(replaceWithPrettyJSON:);
                        break;

                    case iTermSelectionReplacementKindBase64Decode:
                        item.title = NSLocalizedString(@"Replace with Base64-Decoded Value", @"Context menu");
                        item.action = @selector(replaceWithBase64Decoded:);
                        break;

                    case iTermSelectionReplacementKindBase64Encode:
                        item.title = NSLocalizedString(@"Replace with Base64-Encoded Value", @"Context menu");
                        item.action = @selector(replaceWithBase64Encoded:);
                        break;
                }
            }
    ...
    if (shortSelectedText) {
        add(NSLocalizedString(@"Open Selection as URL", @"Context menu"), @selector(browse:));
        if ([[NSWorkspace sharedWorkspace] it_urlIsConditionallyLocallyOpenable:[NSURL URLWithString:shortSelectedText]]) {
            add(NSLocalizedString(@"Open URL in Vertical Split Pane", @"Context menu"), @selector(openURLInVerticalSplitPane:));
            add(NSLocalizedString(@"Open URL in Horizontal Split Pane", @"Context menu"), @selector(openURLInHorizontalSplitPane:));
            [theMenu addItem:[NSMenuItem separatorItem]];
        }
    }
    if (shortSelectedText && [self.delegate contextMenu:self canQuickLookURL:[NSURL URLWithUserSuppliedString:shortSelectedText]]) {
        add(NSLocalizedString(@"Quick Look Link", @"Context menu"), @selector(quickLook:));
    }
    add(NSLocalizedString(@"Search the Web for Selection", @"Context menu"), @selector(searchInBrowser:));

    add(NSLocalizedString(@"Send Email to Selected Address", @"Context menu"), @selector(mail:));
    add(NSLocalizedString(@"Add Trigger…", @"Context menu"), @selector(addTrigger:));
    ...
    // Split pane options
    add(NSLocalizedString(@"Split Pane Vertically", @"Context menu"), @selector(splitTextViewVertically:));
    add(NSLocalizedString(@"Split Pane Horizontally", @"Context menu"), @selector(splitTextViewHorizontally:));
    ...
    add(NSLocalizedString(@"Move Session to Split Pane", @"Context menu"), @selector(movePane:));
    if ([self.delegate contextMenuCurrentTabHasMultipleSessions:self]) {
        NSMenuItem *item = [theMenu addItemWithTitle:NSLocalizedString(@"Move Session to Tab", @"Context menu")
                                              action:@selector(moveSessionToTab:)
                                       keyEquivalent:@""];
        item.representedObject = [self.delegate contextMenuSessionScope:self].ID;
    }
    [theMenu addItemWithTitle:NSLocalizedString(@"Move Session to Window", @"Context menu")
                     action:@selector(moveSessionToWindow:)
                keyEquivalent:@""];
    add(NSLocalizedString(@"Swap With Session…", @"Context menu"), @selector(swapSessions:));

    // Separator
    [theMenu addItem:[NSMenuItem separatorItem]];

    // Copy,  paste, and save
    [theMenu addItemWithTitle:NSLocalizedString(@"Copy", @"Context menu")
    ...
    if (coord.x >= 0 && coord.y >= 0) {
        ...
        if (url) {
            NSMenuItem *item = [theMenu addItemWithTitle:NSLocalizedString(@"Copy Link Address", @"Context menu") action:@selector(copyLinkAddress:) keyEquivalent:@""];
            item.target = self;
            ...
            if (detectedURL) {
                NSMenuItem *item = [theMenu addItemWithTitle:NSLocalizedString(@"Copy URL", @"Context menu") action:@selector(copyDetectedURL:) keyEquivalent:@""];
                item.target = self;
    ...
    [theMenu addItemWithTitle:NSLocalizedString(@"Paste", @"Context menu")
                     action:@selector(paste:) keyEquivalent:@""];
    [theMenu addItemWithTitle:NSLocalizedString(@"Save", @"Context menu")
    ...
    // Select all
    [theMenu addItemWithTitle:NSLocalizedString(@"Select All", @"Context menu")
                     action:@selector(selectAll:) keyEquivalent:@""];

    add(NSLocalizedString(@"Send Selection", @"Context menu"), @selector(sendSelection:));
    add(NSLocalizedString(@"Save Selection as Snippet", @"Context menu"), @selector(saveSelectionAsSnippet:));

    // Clear buffer
    add(NSLocalizedString(@"Clear Buffer", @"Context menu"), @selector(clearTextViewBuffer:));

    // Make note
    add(NSLocalizedString(@"Annotate Selection", @"Context menu"), @selector(addNote:));
    add(NSLocalizedString(@"Reveal Annotation", @"Context menu"), @selector(showNotes:));

    // Separator
    [theMenu addItem:[NSMenuItem separatorItem]];

    // Edit Session
    add(NSLocalizedString(@"Edit Session...", @"Context menu"), @selector(editTextViewSession:));

    // Separator
    [theMenu addItem:[NSMenuItem separatorItem]];

    // Toggle broadcast
    add(NSLocalizedString(@"Toggle Broadcasting Input", @"Context menu"), @selector(toggleBroadcastingInput:));

    // Lock pane
    {
        NSMenuItem *lockItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Lock Pane", @"Context menu")
    ...
    if ([self.delegate contextMenuCurrentTabHasMultipleSessions:self]) {
        ...
        if (allLocked) {
            ...
            NSMenuItem *unlockItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Unlock All Panes in Tab", @"Context menu")
            ...
            NSMenuItem *lockItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Lock All Panes in Tab", @"Context menu")
    ...
    if ([self.delegate contextMenuHasCoprocess:self]) {
        add(NSLocalizedString(@"Stop Coprocess", @"Context menu"), @selector(stopCoprocess:));
    }
    ...
    // Close current pane
    add(NSLocalizedString(@"Close", @"Context menu"), @selector(closeTextViewSession:));
    add(NSLocalizedString(@"Restart", @"Context menu"), @selector(restartSession:));

    [self.delegate contextMenu:self amend:theMenu];

    // Separator
    [theMenu addItem:[NSMenuItem separatorItem]];
    add(NSLocalizedString(@"Bury", @"Context menu"), @selector(bury:));

    // Terminal State
    [theMenu addItem:[NSMenuItem separatorItem]];
    NSMenuItem *terminalState = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Terminal State", @"Context menu") action:nil keyEquivalent:@""];
    terminalState.submenu = [[NSMenu alloc] initWithTitle:NSLocalizedString(@"Terminal State", @"Context menu")];

    struct {
        NSString *title;
        SEL action;
    } terminalStateDecls[] = {
        { NSLocalizedString(@"Alternate Screen", @"Context menu"), @selector(terminalStateToggleAlternateScreen:) },
        { nil, nil },
        { NSLocalizedString(@"Focus Reporting", @"Context menu"), @selector(terminalStateToggleFocusReporting:) },
        { NSLocalizedString(@"Mouse Reporting", @"Context menu"), @selector(terminalStateToggleMouseReporting:) },
        { NSLocalizedString(@"Paste Bracketing", @"Context menu"), @selector(terminalStateTogglePasteBracketing:) },
        { nil, nil },
        { NSLocalizedString(@"Application Cursor", @"Context menu"), @selector(terminalStateToggleApplicationCursor:) },
        { NSLocalizedString(@"Application Keypad", @"Context menu"), @selector(terminalStateToggleApplicationKeypad:) },
        { nil, nil },
        { NSLocalizedString(@"Standard Key Reporting Mode", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"modifyOtherKeys Mode 1", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"modifyOtherKeys Mode 2", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"CSI u Mode", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"Raw Key Reporting Mode", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { nil, nil },
        { NSLocalizedString(@"Disambiguate Escape", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"Report All Event Types", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"Report Alternate Keys", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"Report All Keys as Escape Codes", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { NSLocalizedString(@"Report Associated Text", @"Context menu"), @selector(terminalToggleKeyboardMode:) },
        { nil, nil },
        { NSLocalizedString(@"Literal Controls", @"Context menu"), @selector(terminalStateToggleLiteralMode:)},
        { nil, nil },
        { NSLocalizedString(@"Emulation Level", @"Context menu"), nil }
    };
    NSInteger j = 1;
...
- (void)addMainMenuIfNeededTo:(NSMenu *)menu {
    ...
    NSMenuItem *mainMenuItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Main Menu", @"Context menu") action:nil keyEquivalent:@""];
...
- (NSMenu *)menuForMark:(id<VT100ScreenMarkReading>)mark directory:(NSString *)directory {
    ...
    theItem = [[NSMenuItem alloc] init];
    theItem.title = [NSString stringWithFormat:NSLocalizedString(@"Return code: %d", @"Context menu"), mark.code];
    [theMenu addItem:theItem];
    ...
    if (mark.startDate) {
        ...
        if (hours > 0) {
            theItem.title = [NSString stringWithFormat:NSLocalizedString(@"Running time: %d:%02d:%02d", @"Context menu"),
                             hours, minutes, seconds];
        } else {
            theItem.title = [NSString stringWithFormat:NSLocalizedString(@"Running time: %d:%02d.%03d", @"Context menu"),
    ...
    theItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Re-run Command", @"Context menu")
    ...
    theItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Select Command Output", @"Context menu")
