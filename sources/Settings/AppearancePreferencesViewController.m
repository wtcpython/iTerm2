    ...
    info.customSettingChangedHandler = ^(id sender) {
        ...
        if (isOn) {
            iTermWarningSelection selection =
                [iTermWarning showWarningWithTitle:NSLocalizedString(@"When iTerm2 is excluded from the dock, you can always get back to Preferences using the status bar item. Look for an iTerm2 icon on the right side of your menu bar.", @"UI") actions:@[ NSLocalizedString(@"Exclude From Dock and App Switcher", @"UI"), NSLocalizedString(@"Cancel", @"UI") ]
                                        identifier:nil
